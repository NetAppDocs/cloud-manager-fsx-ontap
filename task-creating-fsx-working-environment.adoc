---
sidebar: sidebar
permalink: task-creating-fsx-working-environment.html
...
. Leave _CIDR Range_ empty and click *Next* to automatically set an available range. Optionally, you can use link:task-setting-up-transit-gateway.html[AWS Transit Gateway] to manually configure a range.
+
...
. Accept the default AWS master key or click *Change Key* to select a different AWS Customer Master Key (CMK). For more information on CMK, see link:https://docs.netapp.com/us-en/occm/task-setting-up-kms.html[Setting up the AWS KMS]. Click *Next* to continue.
+
